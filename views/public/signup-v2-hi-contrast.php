  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .skip-link {
      ...
      background: var(--maap-primary); color: #fff;
    ...
    /* Stronger scrim so white text clears 4.5:1 over any region of the photo.
       Worst case (top, over a bright region) resolves to ~#2e3d4b; near-opaque
       white text on that is >7:1. */
    .hero__bg::after {
      content: ''; position: absolute; inset: 0;
      background: linear-gradient(180deg, rgba(11,17,32,.72) 0%, rgba(11,17,32,.9) 100%);
    }
    .hero__content {
    ...
    .eyebrow {
      ...
      border-radius: 100px; background: rgba(59,130,246,.12);
    }
    /* Solid-white fallback first; the gradient-clip is layered on only where
       supported, and reset under forced-colors so the title never goes invisible. */
    .lead-title {
    ...
    .signin-line { margin: 1.15rem 0 0; font-size: .95rem; color: rgba(255,255,255,.85); text-shadow: 0 1px 8px rgba(0,0,0,.5); }
    .signin-line a { color: #fff; font-weight: 700; text-decoration: underline; text-underline-offset: 3px; }

    /* Right column — dark glass timeline (dark scrim guarantees text contrast) */
    .glass {
    ...
    .t-item p { font-size: .92rem; line-height: 1.55; color: rgba(255,255,255,.9); margin: 0; }
    ...
    /* ───────── slim footer ───────── */
    .footer {
      background: var(--maap-space-900); color: rgba(255,255,255,.7);
    ...
    /* Once registration is initiated this sub-copy carries the key message,
       so it reads at full strength (vs .92 opacity in the new-user state). */
    ...
    @media (max-width: 820px) {
      ...
      /* Keep nav links reachable on mobile: wrap them to a full-width row
         instead of hiding them (no toggle needed for three links). */
      .navbar__inner { flex-wrap: wrap; row-gap: .6rem; }
      .navbar__items { order: 3; width: 100%; justify-content: center; gap: 1.5rem; }
    }
  </style>
</head>
<body>
  ...
  <header class="navbar">
    <div class="navbar__inner">
      ...
      <nav class="navbar__items" aria-label="Primary">
        ...
        <div class="split">
          ...
          <aside class="glass">
            ...
            <ol class="timeline" role="list">
              <li class="t-item s1 is-current" aria-current="step">
                <span class="t-num" aria-hidden="true">1</span>
                ...
                <span class="t-state sr-only">Current step</span>
              </li>
              <li class="t-item s2">
                <span class="t-num" aria-hidden="true">2</span>
                ...
                <span class="t-state sr-only">Upcoming</span>
              </li>
              <li class="t-item s3">
                <span class="t-num" aria-hidden="true">3</span>
            ...
            <div class="mailnote">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false"><path d="M4 5h16v14H4z" stroke="#66c2e9" stroke-width="2"/><path d="M4 6l8 6 8-6" stroke="#66c2e9" stroke-width="2"/></svg>
  ...
  <script>
    ...
    if (new URLSearchParams(location.search).get('status') === 'pending') {
      ...
      if (s1) {
        s1.classList.remove('is-current');
        s1.classList.add('is-done');
        s1.removeAttribute('aria-current');
        s1.querySelector('.t-num').innerHTML = '&#10003;';
        var st1 = s1.querySelector('.t-state'); if (st1) { st1.textContent = 'Completed'; }
      }
      if (s2) {
        s2.classList.add('is-current');
        s2.setAttribute('aria-current', 'step');
        var st2 = s2.querySelector('.t-state'); if (st2) { st2.textContent = 'Current step'; }
      }
    }
  </script>
</body>
